...
from snmodel import DCGenerator, DCDiscriminator
...
def training_loop(train_dataloader, opts):
    ...
    for _ in range(opts.num_epochs):
        ...
        for batch in train_dataloader:
            ...
            D_real_loss = -torch.mean(torch.log(torch.sigmoid(D(real_images))))
            ...
            D_fake_loss = -torch.mean(torch.log(1-torch.sigmoid(D(fake_images.detach()))))
            ...
            G_loss = -torch.mean(torch.log(torch.sigmoid(D(fake_images))))
